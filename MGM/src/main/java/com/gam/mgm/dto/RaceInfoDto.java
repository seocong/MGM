package com.gam.mgm.dto;

import java.util.Date;

public class RaceInfoDto {
	private int ri_meet;
	private Date ri_rcDate;
	private int ri_rcNo;
	private int ri_ilsu;
	private int ri_rcDist;
	private String ri_rank;
	private String ri_budam;
	private String ri_rcName;
	private String ri_ageCond;
	private String ri_weather;
	private String ri_track;
	private int ri_chaksun1;
	private int ri_chaksun2;
	private int ri_chaksun3;
	private int ri_chaksun4;
	private int ri_chaksun5;
	private long ri_winAmt;
	private long ri_plcAmt;
	private long ri_qnlAmt;
	private long ri_exaAmt;
	private long ri_qplAmt;
	private long ri_tlaAmt;
	private long ri_triAmt;
	private long ri_totalAmt;
	private int ri_cnt;
	private String ri_winChulNo;
	private String ri_winOdds;
	private String ri_plcChulNo;
	private String ri_plcOdds;
	private String ri_qnlChulNo;
	private String ri_qnlOdds;
	private String ri_exaChulNo;
	private String ri_exaOdds;
	private String ri_qplChulNo;
	private String ri_qplOdds;
	private String ri_tlaChulNo;
	private String ri_tlaOdds;
	private String ri_triChulNo;
	private String ri_triOdds;
	private int ri_dist_1f;
	private int ri_dist_2f;
	private int ri_dist_3f;
	private int ri_dist_4f;
	private int ri_dist_5f;
	private int ri_dist_6f;
	private int ri_dist_7f;
	private int ri_dist_8f;
	private int ri_dist_9f;
	private int ri_dist_10f;
	private double ri_time_1f;
	private double ri_time_2f;
	private double ri_time_3f;
	private double ri_time_4f;
	private double ri_time_5f;
	private double ri_time_6f;
	private double ri_time_7f;
	private double ri_time_8f;
	private double ri_time_9f;
	private double ri_time_10f;
	private double ri_time_11f;
	private double ri_time_12f;
	private double ri_passTime_1f;
	private double ri_passTime_2f;
	private double ri_passTime_3f;
	private double ri_passTime_4f;
	private double ri_passTime_5f;
	private double ri_passTime_6f;
	private double ri_passTime_7f;
	private double ri_passTime_8f;
	private double ri_passTime_9f;
	private double ri_passTime_10f;
	private String ri_passrankS1f;
	private String ri_passrankG8f_1c;
	private String ri_passrankG6f_2c;
	private String ri_passrankG4f_3c;
	private String ri_passrankG3f_4c;
	private String ri_passrankG2f;
	private String ri_passrankG1f;
	public RaceInfoDto() {
		super();
		// TODO Auto-generated constructor stub
	}
	public RaceInfoDto(int ri_meet, Date ri_rcDate, int ri_rcNo, int ri_ilsu, int ri_rcDist, String ri_rank,
			String ri_budam, String ri_rcName, String ri_ageCond, String ri_weather, String ri_track, int ri_chaksun1,
			int ri_chaksun2, int ri_chaksun3, int ri_chaksun4, int ri_chaksun5, long ri_winAmt, long ri_plcAmt,
			long ri_qnlAmt, long ri_exaAmt, long ri_qplAmt, long ri_tlaAmt, long ri_triAmt, long ri_totalAmt,
			int ri_cnt, String ri_winChulNo, String ri_winOdds, String ri_plcChulNo, String ri_plcOdds,
			String ri_qnlChulNo, String ri_qnlOdds, String ri_exaChulNo, String ri_exaOdds, String ri_qplChulNo,
			String ri_qplOdds, String ri_tlaChulNo, String ri_tlaOdds, String ri_triChulNo, String ri_triOdds,
			int ri_dist_1f, int ri_dist_2f, int ri_dist_3f, int ri_dist_4f, int ri_dist_5f, int ri_dist_6f,
			int ri_dist_7f, int ri_dist_8f, int ri_dist_9f, int ri_dist_10f, double ri_time_1f, double ri_time_2f,
			double ri_time_3f, double ri_time_4f, double ri_time_5f, double ri_time_6f, double ri_time_7f, double ri_time_8f,
			double ri_time_9f, double ri_time_10f, double ri_time_11f, double ri_time_12f, double ri_passTime_1f,
			double ri_passTime_2f, double ri_passTime_3f, double ri_passTime_4f, double ri_passTime_5f,
			double ri_passTime_6f, double ri_passTime_7f, double ri_passTime_8f, double ri_passTime_9f,
			double ri_passTime_10f, String ri_passrankS1f, String ri_passrankG8f_1c, String ri_passrankG6f_2c,
			String ri_passrankG4f_3c, String ri_passrankG3f_4c, String ri_passrankG2f, String ri_passrankG1f) {
		super();
		this.ri_meet = ri_meet;
		this.ri_rcDate = ri_rcDate;
		this.ri_rcNo = ri_rcNo;
		this.ri_ilsu = ri_ilsu;
		this.ri_rcDist = ri_rcDist;
		this.ri_rank = ri_rank;
		this.ri_budam = ri_budam;
		this.ri_rcName = ri_rcName;
		this.ri_ageCond = ri_ageCond;
		this.ri_weather = ri_weather;
		this.ri_track = ri_track;
		this.ri_chaksun1 = ri_chaksun1;
		this.ri_chaksun2 = ri_chaksun2;
		this.ri_chaksun3 = ri_chaksun3;
		this.ri_chaksun4 = ri_chaksun4;
		this.ri_chaksun5 = ri_chaksun5;
		this.ri_winAmt = ri_winAmt;
		this.ri_plcAmt = ri_plcAmt;
		this.ri_qnlAmt = ri_qnlAmt;
		this.ri_exaAmt = ri_exaAmt;
		this.ri_qplAmt = ri_qplAmt;
		this.ri_tlaAmt = ri_tlaAmt;
		this.ri_triAmt = ri_triAmt;
		this.ri_totalAmt = ri_totalAmt;
		this.ri_cnt = ri_cnt;
		this.ri_winChulNo = ri_winChulNo;
		this.ri_winOdds = ri_winOdds;
		this.ri_plcChulNo = ri_plcChulNo;
		this.ri_plcOdds = ri_plcOdds;
		this.ri_qnlChulNo = ri_qnlChulNo;
		this.ri_qnlOdds = ri_qnlOdds;
		this.ri_exaChulNo = ri_exaChulNo;
		this.ri_exaOdds = ri_exaOdds;
		this.ri_qplChulNo = ri_qplChulNo;
		this.ri_qplOdds = ri_qplOdds;
		this.ri_tlaChulNo = ri_tlaChulNo;
		this.ri_tlaOdds = ri_tlaOdds;
		this.ri_triChulNo = ri_triChulNo;
		this.ri_triOdds = ri_triOdds;
		this.ri_dist_1f = ri_dist_1f;
		this.ri_dist_2f = ri_dist_2f;
		this.ri_dist_3f = ri_dist_3f;
		this.ri_dist_4f = ri_dist_4f;
		this.ri_dist_5f = ri_dist_5f;
		this.ri_dist_6f = ri_dist_6f;
		this.ri_dist_7f = ri_dist_7f;
		this.ri_dist_8f = ri_dist_8f;
		this.ri_dist_9f = ri_dist_9f;
		this.ri_dist_10f = ri_dist_10f;
		this.ri_time_1f = ri_time_1f;
		this.ri_time_2f = ri_time_2f;
		this.ri_time_3f = ri_time_3f;
		this.ri_time_4f = ri_time_4f;
		this.ri_time_5f = ri_time_5f;
		this.ri_time_6f = ri_time_6f;
		this.ri_time_7f = ri_time_7f;
		this.ri_time_8f = ri_time_8f;
		this.ri_time_9f = ri_time_9f;
		this.ri_time_10f = ri_time_10f;
		this.ri_time_11f = ri_time_11f;
		this.ri_time_12f = ri_time_12f;
		this.ri_passTime_1f = ri_passTime_1f;
		this.ri_passTime_2f = ri_passTime_2f;
		this.ri_passTime_3f = ri_passTime_3f;
		this.ri_passTime_4f = ri_passTime_4f;
		this.ri_passTime_5f = ri_passTime_5f;
		this.ri_passTime_6f = ri_passTime_6f;
		this.ri_passTime_7f = ri_passTime_7f;
		this.ri_passTime_8f = ri_passTime_8f;
		this.ri_passTime_9f = ri_passTime_9f;
		this.ri_passTime_10f = ri_passTime_10f;
		this.ri_passrankS1f = ri_passrankS1f;
		this.ri_passrankG8f_1c = ri_passrankG8f_1c;
		this.ri_passrankG6f_2c = ri_passrankG6f_2c;
		this.ri_passrankG4f_3c = ri_passrankG4f_3c;
		this.ri_passrankG3f_4c = ri_passrankG3f_4c;
		this.ri_passrankG2f = ri_passrankG2f;
		this.ri_passrankG1f = ri_passrankG1f;
	}
	@Override
	public String toString() {
		return "RaceInfoDto [ri_meet=" + ri_meet + ", ri_rcDate=" + ri_rcDate + ", ri_rcNo=" + ri_rcNo + ", ri_ilsu="
				+ ri_ilsu + ", ri_rcDist=" + ri_rcDist + ", ri_rank=" + ri_rank + ", ri_budam=" + ri_budam
				+ ", ri_rcName=" + ri_rcName + ", ri_ageCond=" + ri_ageCond + ", ri_weather=" + ri_weather
				+ ", ri_track=" + ri_track + ", ri_chaksun1=" + ri_chaksun1 + ", ri_chaksun2=" + ri_chaksun2
				+ ", ri_chaksun3=" + ri_chaksun3 + ", ri_chaksun4=" + ri_chaksun4 + ", ri_chaksun5=" + ri_chaksun5
				+ ", ri_winAmt=" + ri_winAmt + ", ri_plcAmt=" + ri_plcAmt + ", ri_qnlAmt=" + ri_qnlAmt + ", ri_exaAmt="
				+ ri_exaAmt + ", ri_qplAmt=" + ri_qplAmt + ", ri_tlaAmt=" + ri_tlaAmt + ", ri_triAmt=" + ri_triAmt
				+ ", ri_totalAmt=" + ri_totalAmt + ", ri_cnt=" + ri_cnt + ", ri_winChulNo=" + ri_winChulNo
				+ ", ri_winOdds=" + ri_winOdds + ", ri_plcChulNo=" + ri_plcChulNo + ", ri_plcOdds=" + ri_plcOdds
				+ ", ri_qnlChulNo=" + ri_qnlChulNo + ", ri_qnlOdds=" + ri_qnlOdds + ", ri_exaChulNo=" + ri_exaChulNo
				+ ", ri_exaOdds=" + ri_exaOdds + ", ri_qplChulNo=" + ri_qplChulNo + ", ri_qplOdds=" + ri_qplOdds
				+ ", ri_tlaChulNo=" + ri_tlaChulNo + ", ri_tlaOdds=" + ri_tlaOdds + ", ri_triChulNo=" + ri_triChulNo
				+ ", ri_triOdds=" + ri_triOdds + ", ri_dist_1f=" + ri_dist_1f + ", ri_dist_2f=" + ri_dist_2f
				+ ", ri_dist_3f=" + ri_dist_3f + ", ri_dist_4f=" + ri_dist_4f + ", ri_dist_5f=" + ri_dist_5f
				+ ", ri_dist_6f=" + ri_dist_6f + ", ri_dist_7f=" + ri_dist_7f + ", ri_dist_8f=" + ri_dist_8f
				+ ", ri_dist_9f=" + ri_dist_9f + ", ri_dist_10f=" + ri_dist_10f + ", ri_time_1f=" + ri_time_1f
				+ ", ri_time_2f=" + ri_time_2f + ", ri_time_3f=" + ri_time_3f + ", ri_time_4f=" + ri_time_4f
				+ ", ri_time_5f=" + ri_time_5f + ", ri_time_6f=" + ri_time_6f + ", ri_time_7f=" + ri_time_7f
				+ ", ri_time_8f=" + ri_time_8f + ", ri_time_9f=" + ri_time_9f + ", ri_time_10f=" + ri_time_10f
				+ ", ri_time_11f=" + ri_time_11f + ", ri_time_12f=" + ri_time_12f + ", ri_passTime_1f=" + ri_passTime_1f
				+ ", ri_passTime_2f=" + ri_passTime_2f + ", ri_passTime_3f=" + ri_passTime_3f + ", ri_passTime_4f="
				+ ri_passTime_4f + ", ri_passTime_5f=" + ri_passTime_5f + ", ri_passTime_6f=" + ri_passTime_6f
				+ ", ri_passTime_7f=" + ri_passTime_7f + ", ri_passTime_8f=" + ri_passTime_8f + ", ri_passTime_9f="
				+ ri_passTime_9f + ", ri_passTime_10f=" + ri_passTime_10f + ", ri_passrankS1f=" + ri_passrankS1f
				+ ", ri_passrankG8f_1c=" + ri_passrankG8f_1c + ", ri_passrankG6f_2c=" + ri_passrankG6f_2c
				+ ", ri_passrankG4f_3c=" + ri_passrankG4f_3c + ", ri_passrankG3f_4c=" + ri_passrankG3f_4c
				+ ", ri_passrankG2f=" + ri_passrankG2f + ", ri_passrankG1f=" + ri_passrankG1f + "]";
	}
	public int getRi_meet() {
		return ri_meet;
	}
	public void setRi_meet(int ri_meet) {
		this.ri_meet = ri_meet;
	}
	public Date getRi_rcDate() {
		return ri_rcDate;
	}
	public void setRi_rcDate(Date ri_rcDate) {
		this.ri_rcDate = ri_rcDate;
	}
	public int getRi_rcNo() {
		return ri_rcNo;
	}
	public void setRi_rcNo(int ri_rcNo) {
		this.ri_rcNo = ri_rcNo;
	}
	public int getRi_ilsu() {
		return ri_ilsu;
	}
	public void setRi_ilsu(int ri_ilsu) {
		this.ri_ilsu = ri_ilsu;
	}
	public int getRi_rcDist() {
		return ri_rcDist;
	}
	public void setRi_rcDist(int ri_rcDist) {
		this.ri_rcDist = ri_rcDist;
	}
	public String getRi_rank() {
		return ri_rank;
	}
	public void setRi_rank(String ri_rank) {
		this.ri_rank = ri_rank;
	}
	public String getRi_budam() {
		return ri_budam;
	}
	public void setRi_budam(String ri_budam) {
		this.ri_budam = ri_budam;
	}
	public String getRi_rcName() {
		return ri_rcName;
	}
	public void setRi_rcName(String ri_rcName) {
		this.ri_rcName = ri_rcName;
	}
	public String getRi_ageCond() {
		return ri_ageCond;
	}
	public void setRi_ageCond(String ri_ageCond) {
		this.ri_ageCond = ri_ageCond;
	}
	public String getRi_weather() {
		return ri_weather;
	}
	public void setRi_weather(String ri_weather) {
		this.ri_weather = ri_weather;
	}
	public String getRi_track() {
		return ri_track;
	}
	public void setRi_track(String ri_track) {
		this.ri_track = ri_track;
	}
	public int getRi_chaksun1() {
		return ri_chaksun1;
	}
	public void setRi_chaksun1(int ri_chaksun1) {
		this.ri_chaksun1 = ri_chaksun1;
	}
	public int getRi_chaksun2() {
		return ri_chaksun2;
	}
	public void setRi_chaksun2(int ri_chaksun2) {
		this.ri_chaksun2 = ri_chaksun2;
	}
	public int getRi_chaksun3() {
		return ri_chaksun3;
	}
	public void setRi_chaksun3(int ri_chaksun3) {
		this.ri_chaksun3 = ri_chaksun3;
	}
	public int getRi_chaksun4() {
		return ri_chaksun4;
	}
	public void setRi_chaksun4(int ri_chaksun4) {
		this.ri_chaksun4 = ri_chaksun4;
	}
	public int getRi_chaksun5() {
		return ri_chaksun5;
	}
	public void setRi_chaksun5(int ri_chaksun5) {
		this.ri_chaksun5 = ri_chaksun5;
	}
	public long getRi_winAmt() {
		return ri_winAmt;
	}
	public void setRi_winAmt(long ri_winAmt) {
		this.ri_winAmt = ri_winAmt;
	}
	public long getRi_plcAmt() {
		return ri_plcAmt;
	}
	public void setRi_plcAmt(long ri_plcAmt) {
		this.ri_plcAmt = ri_plcAmt;
	}
	public long getRi_qnlAmt() {
		return ri_qnlAmt;
	}
	public void setRi_qnlAmt(long ri_qnlAmt) {
		this.ri_qnlAmt = ri_qnlAmt;
	}
	public long getRi_exaAmt() {
		return ri_exaAmt;
	}
	public void setRi_exaAmt(long ri_exaAmt) {
		this.ri_exaAmt = ri_exaAmt;
	}
	public long getRi_qplAmt() {
		return ri_qplAmt;
	}
	public void setRi_qplAmt(long ri_qplAmt) {
		this.ri_qplAmt = ri_qplAmt;
	}
	public long getRi_tlaAmt() {
		return ri_tlaAmt;
	}
	public void setRi_tlaAmt(long ri_tlaAmt) {
		this.ri_tlaAmt = ri_tlaAmt;
	}
	public long getRi_triAmt() {
		return ri_triAmt;
	}
	public void setRi_triAmt(long ri_triAmt) {
		this.ri_triAmt = ri_triAmt;
	}
	public long getRi_totalAmt() {
		return ri_totalAmt;
	}
	public void setRi_totalAmt(long ri_totalAmt) {
		this.ri_totalAmt = ri_totalAmt;
	}
	public int getRi_cnt() {
		return ri_cnt;
	}
	public void setRi_cnt(int ri_cnt) {
		this.ri_cnt = ri_cnt;
	}
	public String getRi_winChulNo() {
		return ri_winChulNo;
	}
	public void setRi_winChulNo(String ri_winChulNo) {
		this.ri_winChulNo = ri_winChulNo;
	}
	public String getRi_winOdds() {
		return ri_winOdds;
	}
	public void setRi_winOdds(String ri_winOdds) {
		this.ri_winOdds = ri_winOdds;
	}
	public String getRi_plcChulNo() {
		return ri_plcChulNo;
	}
	public void setRi_plcChulNo(String ri_plcChulNo) {
		this.ri_plcChulNo = ri_plcChulNo;
	}
	public String getRi_plcOdds() {
		return ri_plcOdds;
	}
	public void setRi_plcOdds(String ri_plcOdds) {
		this.ri_plcOdds = ri_plcOdds;
	}
	public String getRi_qnlChulNo() {
		return ri_qnlChulNo;
	}
	public void setRi_qnlChulNo(String ri_qnlChulNo) {
		this.ri_qnlChulNo = ri_qnlChulNo;
	}
	public String getRi_qnlOdds() {
		return ri_qnlOdds;
	}
	public void setRi_qnlOdds(String ri_qnlOdds) {
		this.ri_qnlOdds = ri_qnlOdds;
	}
	public String getRi_exaChulNo() {
		return ri_exaChulNo;
	}
	public void setRi_exaChulNo(String ri_exaChulNo) {
		this.ri_exaChulNo = ri_exaChulNo;
	}
	public String getRi_exaOdds() {
		return ri_exaOdds;
	}
	public void setRi_exaOdds(String ri_exaOdds) {
		this.ri_exaOdds = ri_exaOdds;
	}
	public String getRi_qplChulNo() {
		return ri_qplChulNo;
	}
	public void setRi_qplChulNo(String ri_qplChulNo) {
		this.ri_qplChulNo = ri_qplChulNo;
	}
	public String getRi_qplOdds() {
		return ri_qplOdds;
	}
	public void setRi_qplOdds(String ri_qplOdds) {
		this.ri_qplOdds = ri_qplOdds;
	}
	public String getRi_tlaChulNo() {
		return ri_tlaChulNo;
	}
	public void setRi_tlaChulNo(String ri_tlaChulNo) {
		this.ri_tlaChulNo = ri_tlaChulNo;
	}
	public String getRi_tlaOdds() {
		return ri_tlaOdds;
	}
	public void setRi_tlaOdds(String ri_tlaOdds) {
		this.ri_tlaOdds = ri_tlaOdds;
	}
	public String getRi_triChulNo() {
		return ri_triChulNo;
	}
	public void setRi_triChulNo(String ri_triChulNo) {
		this.ri_triChulNo = ri_triChulNo;
	}
	public String getRi_triOdds() {
		return ri_triOdds;
	}
	public void setRi_triOdds(String ri_triOdds) {
		this.ri_triOdds = ri_triOdds;
	}
	public int getRi_dist_1f() {
		return ri_dist_1f;
	}
	public void setRi_dist_1f(int ri_dist_1f) {
		this.ri_dist_1f = ri_dist_1f;
	}
	public int getRi_dist_2f() {
		return ri_dist_2f;
	}
	public void setRi_dist_2f(int ri_dist_2f) {
		this.ri_dist_2f = ri_dist_2f;
	}
	public int getRi_dist_3f() {
		return ri_dist_3f;
	}
	public void setRi_dist_3f(int ri_dist_3f) {
		this.ri_dist_3f = ri_dist_3f;
	}
	public int getRi_dist_4f() {
		return ri_dist_4f;
	}
	public void setRi_dist_4f(int ri_dist_4f) {
		this.ri_dist_4f = ri_dist_4f;
	}
	public int getRi_dist_5f() {
		return ri_dist_5f;
	}
	public void setRi_dist_5f(int ri_dist_5f) {
		this.ri_dist_5f = ri_dist_5f;
	}
	public int getRi_dist_6f() {
		return ri_dist_6f;
	}
	public void setRi_dist_6f(int ri_dist_6f) {
		this.ri_dist_6f = ri_dist_6f;
	}
	public int getRi_dist_7f() {
		return ri_dist_7f;
	}
	public void setRi_dist_7f(int ri_dist_7f) {
		this.ri_dist_7f = ri_dist_7f;
	}
	public int getRi_dist_8f() {
		return ri_dist_8f;
	}
	public void setRi_dist_8f(int ri_dist_8f) {
		this.ri_dist_8f = ri_dist_8f;
	}
	public int getRi_dist_9f() {
		return ri_dist_9f;
	}
	public void setRi_dist_9f(int ri_dist_9f) {
		this.ri_dist_9f = ri_dist_9f;
	}
	public int getRi_dist_10f() {
		return ri_dist_10f;
	}
	public void setRi_dist_10f(int ri_dist_10f) {
		this.ri_dist_10f = ri_dist_10f;
	}
	public double getRi_time_1f() {
		return ri_time_1f;
	}
	public void setRi_time_1f(double ri_time_1f) {
		this.ri_time_1f = ri_time_1f;
	}
	public double getRi_time_2f() {
		return ri_time_2f;
	}
	public void setRi_time_2f(double ri_time_2f) {
		this.ri_time_2f = ri_time_2f;
	}
	public double getRi_time_3f() {
		return ri_time_3f;
	}
	public void setRi_time_3f(double ri_time_3f) {
		this.ri_time_3f = ri_time_3f;
	}
	public double getRi_time_4f() {
		return ri_time_4f;
	}
	public void setRi_time_4f(double ri_time_4f) {
		this.ri_time_4f = ri_time_4f;
	}
	public double getRi_time_5f() {
		return ri_time_5f;
	}
	public void setRi_time_5f(double ri_time_5f) {
		this.ri_time_5f = ri_time_5f;
	}
	public double getRi_time_6f() {
		return ri_time_6f;
	}
	public void setRi_time_6f(double ri_time_6f) {
		this.ri_time_6f = ri_time_6f;
	}
	public double getRi_time_7f() {
		return ri_time_7f;
	}
	public void setRi_time_7f(double ri_time_7f) {
		this.ri_time_7f = ri_time_7f;
	}
	public double getRi_time_8f() {
		return ri_time_8f;
	}
	public void setRi_time_8f(double ri_time_8f) {
		this.ri_time_8f = ri_time_8f;
	}
	public double getRi_time_9f() {
		return ri_time_9f;
	}
	public void setRi_time_9f(double ri_time_9f) {
		this.ri_time_9f = ri_time_9f;
	}
	public double getRi_time_10f() {
		return ri_time_10f;
	}
	public void setRi_time_10f(double ri_time_10f) {
		this.ri_time_10f = ri_time_10f;
	}
	public double getRi_time_11f() {
		return ri_time_11f;
	}
	public void setRi_time_11f(double ri_time_11f) {
		this.ri_time_11f = ri_time_11f;
	}
	public double getRi_time_12f() {
		return ri_time_12f;
	}
	public void setRi_time_12f(double ri_time_12f) {
		this.ri_time_12f = ri_time_12f;
	}
	public double getRi_passTime_1f() {
		return ri_passTime_1f;
	}
	public void setRi_passTime_1f(double ri_passTime_1f) {
		this.ri_passTime_1f = ri_passTime_1f;
	}
	public double getRi_passTime_2f() {
		return ri_passTime_2f;
	}
	public void setRi_passTime_2f(double ri_passTime_2f) {
		this.ri_passTime_2f = ri_passTime_2f;
	}
	public double getRi_passTime_3f() {
		return ri_passTime_3f;
	}
	public void setRi_passTime_3f(double ri_passTime_3f) {
		this.ri_passTime_3f = ri_passTime_3f;
	}
	public double getRi_passTime_4f() {
		return ri_passTime_4f;
	}
	public void setRi_passTime_4f(double ri_passTime_4f) {
		this.ri_passTime_4f = ri_passTime_4f;
	}
	public double getRi_passTime_5f() {
		return ri_passTime_5f;
	}
	public void setRi_passTime_5f(double ri_passTime_5f) {
		this.ri_passTime_5f = ri_passTime_5f;
	}
	public double getRi_passTime_6f() {
		return ri_passTime_6f;
	}
	public void setRi_passTime_6f(double ri_passTime_6f) {
		this.ri_passTime_6f = ri_passTime_6f;
	}
	public double getRi_passTime_7f() {
		return ri_passTime_7f;
	}
	public void setRi_passTime_7f(double ri_passTime_7f) {
		this.ri_passTime_7f = ri_passTime_7f;
	}
	public double getRi_passTime_8f() {
		return ri_passTime_8f;
	}
	public void setRi_passTime_8f(double ri_passTime_8f) {
		this.ri_passTime_8f = ri_passTime_8f;
	}
	public double getRi_passTime_9f() {
		return ri_passTime_9f;
	}
	public void setRi_passTime_9f(double ri_passTime_9f) {
		this.ri_passTime_9f = ri_passTime_9f;
	}
	public double getRi_passTime_10f() {
		return ri_passTime_10f;
	}
	public void setRi_passTime_10f(double ri_passTime_10f) {
		this.ri_passTime_10f = ri_passTime_10f;
	}
	public String getRi_passrankS1f() {
		return ri_passrankS1f;
	}
	public void setRi_passrankS1f(String ri_passrankS1f) {
		this.ri_passrankS1f = ri_passrankS1f;
	}
	public String getRi_passrankG8f_1c() {
		return ri_passrankG8f_1c;
	}
	public void setRi_passrankG8f_1c(String ri_passrankG8f_1c) {
		this.ri_passrankG8f_1c = ri_passrankG8f_1c;
	}
	public String getRi_passrankG6f_2c() {
		return ri_passrankG6f_2c;
	}
	public void setRi_passrankG6f_2c(String ri_passrankG6f_2c) {
		this.ri_passrankG6f_2c = ri_passrankG6f_2c;
	}
	public String getRi_passrankG4f_3c() {
		return ri_passrankG4f_3c;
	}
	public void setRi_passrankG4f_3c(String ri_passrankG4f_3c) {
		this.ri_passrankG4f_3c = ri_passrankG4f_3c;
	}
	public String getRi_passrankG3f_4c() {
		return ri_passrankG3f_4c;
	}
	public void setRi_passrankG3f_4c(String ri_passrankG3f_4c) {
		this.ri_passrankG3f_4c = ri_passrankG3f_4c;
	}
	public String getRi_passrankG2f() {
		return ri_passrankG2f;
	}
	public void setRi_passrankG2f(String ri_passrankG2f) {
		this.ri_passrankG2f = ri_passrankG2f;
	}
	public String getRi_passrankG1f() {
		return ri_passrankG1f;
	}
	public void setRi_passrankG1f(String ri_passrankG1f) {
		this.ri_passrankG1f = ri_passrankG1f;
	}
	
}
